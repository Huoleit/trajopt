...
osg::Drawable* toOsgDrawable(const KinBody::Link::TRIMESH& mesh) {
  ...
  for (int i = 0; i < mesh.indices.size(); ++i) deui->push_back(mesh.indices[i]);
...
class SetTransparencyVisitor : public osg::NodeVisitor {
 public:
  float alpha;
  SetTransparencyVisitor(float _alpha) : alpha(_alpha) { setTraversalMode(osg::NodeVisitor::TRAVERSE_ALL_CHILDREN); }
  void apply(osg::Geode& geode) {
    StateSet* ss = geode.getOrCreateStateSet();
    osg::Material* mat = static_cast<Material*>(ss->getAttribute(StateAttribute::MATERIAL));
    if (!mat) {
...
class RefPtrHolder : public UserData {
 public:
  ...
  RefPtrHolder(osg::Referenced* x) { rp = x; }
};
...
class OsgGraphHandle : public OpenRAVE::GraphHandle {
 public:
  ...
  OsgGraphHandle(osg::Node* _node, osg::Group* _parent) : parent(_parent), node(_node) { parent->addChild(node); }
  ~OsgGraphHandle() { parent->removeChild(node); }
};

KinBodyGroup* GetOsgGroup(KinBody& body) {
  UserDataPtr rph = trajopt::GetUserData(body, "osg");
  return rph ? static_cast<KinBodyGroup*>(static_cast<RefPtrHolder*>(rph.get())->rp.get()) : NULL;
}
...
OSGViewer::OSGViewer(EnvironmentBasePtr env) : ViewerBase(env), m_idling(false) {
  ...
  m_handler->setHomePosition(osg::Vec3d(3.5, 0, 2), osg::Vec3d(-2, 0, 0), osg::Vec3d(0, 0, 1));
...
void OSGViewer::TakeScreenshot(const std::string& filename) {
  osg::ref_ptr<SnapImageDrawCallback> snapImageDrawCallback =
      dynamic_cast<SnapImageDrawCallback*>(m_cam->getPostDrawCallback());
...
bool OSGViewer::GetLastScreenshot(std::vector<unsigned char>& image, unsigned int& height, unsigned int& width) {
  osg::ref_ptr<SnapImageDrawCallback> snapImageDrawCallback =
      dynamic_cast<SnapImageDrawCallback*>(m_cam->getPostDrawCallback());
  if (!snapImageDrawCallback.get()) return false;
...
OpenRAVE::GraphHandlePtr OSGViewer::drawarrow(const RaveVectorf& p1, const RaveVectorf& p2, float fwidth,
...
OpenRAVE::GraphHandlePtr OSGViewer::plot3(const float* ppoints, int numPoints, int stride, float fPointSize,
                                          const RaveVector<float>& color, int drawstyle) {
...
OpenRAVE::GraphHandlePtr OSGViewer::plot3(const float* ppoints, int numPoints, int stride, float pointsize,
                                          const float* colors, int drawstyle, bool bhasalpha) {
  osg::Geometry* geom = new osg::Geometry;
...
OpenRAVE::GraphHandlePtr OSGViewer::drawtrimesh(const float* ppoints, int stride, const int* pIndices, int numTriangles,
                                                const RaveVectorf& color) {
  osg::DrawElementsUInt* deui = new osg::DrawElementsUInt(GL_TRIANGLES);
  osg::Vec3Array* vec = new osg::Vec3Array();
...
OpenRAVE::GraphHandlePtr OSGViewer::_drawlines(osg::PrimitiveSet::Mode mode, const float* ppoints, int numPoints,
                                               int stride, float fwidth, const RaveVectorf& color) {
  osg::Geometry* geom = new osg::Geometry;
...
OpenRAVE::GraphHandlePtr OSGViewer::drawlinestrip(const float* ppoints, int numPoints, int stride, float fwidth,
                                                  const RaveVectorf& color) {
  return _drawlines(osg::PrimitiveSet::LINE_STRIP, ppoints, numPoints, stride, fwidth, color);
}
OpenRAVE::GraphHandlePtr OSGViewer::drawlinelist(const float* ppoints, int numPoints, int stride, float fwidth,
                                                 const RaveVectorf& color) {
  return _drawlines(osg::PrimitiveSet::LINES, ppoints, numPoints, stride, fwidth, color);
}

OpenRAVE::GraphHandlePtr OSGViewer::drawtext(const std::string& text, float x, float y, float fontsize,
