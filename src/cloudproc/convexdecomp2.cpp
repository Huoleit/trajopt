...
struct Frontier {
  ...
  void exclude(const T& x) { m_excluded.insert(x); }
  bool empty() { return m_frontier.empty(); }
};
...
struct AlmostConvex {
  ...
  float deficit(const PointNormal& pn1, const PointNormal& pn2) {
    ...
    return fmaxf(p2minusp1.dot(pn2.getNormalVector3fMap()), -p2minusp1.dot(pn1.getNormalVector3fMap())) /
           p2minusp1.norm();
  }

  float calcMaxDeficit(const PointNormal& pn) {
    float out = 0;
    BOOST_FOREACH (const PointNormal& pn1, m_pns) { out = fmaxf(out, deficit(pn, pn1)); }
    return out;
  }
...
vector<int> getUnlabeled(const vector<int>& nbs, const vector<int>& labels) {
  vector<int> out;
  BOOST_FOREACH (const int& i, nbs)
    if (labels[i] == UNLABELED) out.push_back(i);
  return out;
}
  ...
  while ((i_seed = getNextUnlabeled(labels, i_seed)) != NO_NEXT_LABEL) {
    ...
    while (!frontier.empty()) {
      ...
      if (deficit < thresh) {
        ...
        vector<int> unlabeled_nbs = getUnlabeled(nbs, labels);
        frontier.push(unlabeled_nbs);
        //        BOOST_FOREACH(int i, unlabeled_nbs) assert( (cloud->points[i].getVector3fMap() -
        //        cloud->points[i_cur].getVector3fMap()).norm() < nb_rad );
        DEBUG_PRINT("added %i\n", i_cur);
