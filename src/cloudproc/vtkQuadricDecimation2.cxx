...
void vtkQuadricDecimation2::SetPointAttributeArray(vtkIdType ptId, const double* x) {
  int i;
  this->Mesh->GetPoints()->SetPoint(ptId, x);

  for (i = 0; i < this->NumberOfComponents; i++) {
    if (i < this->AttributeComponents[0]) {
      this->Mesh->GetPointData()->GetScalars()->SetComponent(ptId, i, x[3 + i] / this->AttributeScale[0]);
    } else if (i < this->AttributeComponents[1]) {
      this->Mesh->GetPointData()->GetVectors()->SetComponent(ptId, i - this->AttributeComponents[0],
                                                             x[3 + i] / this->AttributeScale[1]);
    } else if (i < this->AttributeComponents[2]) {
      this->Mesh->GetPointData()->GetNormals()->SetComponent(ptId, i - this->AttributeComponents[1],
                                                             x[3 + i] / this->AttributeScale[2]);
    } else if (i < this->AttributeComponents[3]) {
      this->Mesh->GetPointData()->GetTCoords()->SetComponent(ptId, i - this->AttributeComponents[2],
                                                             x[3 + i] / this->AttributeScale[3]);
    } else if (i < this->AttributeComponents[4]) {
      this->Mesh->GetPointData()->GetTensors()->SetComponent(ptId, i - this->AttributeComponents[3],
                                                             x[3 + i] / this->AttributeScale[4]);
    }
  }
}
...
void vtkQuadricDecimation2::GetPointAttributeArray(vtkIdType ptId, double* x) {
  ...
  for (i = 0; i < this->NumberOfComponents; i++) {
    if (i < this->AttributeComponents[0]) {
      x[3 + i] = this->Mesh->GetPointData()->GetScalars()->GetComponent(ptId, i) * this->AttributeScale[0];
    } else if (i < this->AttributeComponents[1]) {
      x[3 + i] = this->Mesh->GetPointData()->GetVectors()->GetComponent(ptId, i - this->AttributeComponents[0]) *
                 this->AttributeScale[1];
    } else if (i < this->AttributeComponents[2]) {
      x[3 + i] = this->Mesh->GetPointData()->GetNormals()->GetComponent(ptId, i - this->AttributeComponents[1]) *
                 this->AttributeScale[2];
    } else if (i < this->AttributeComponents[3]) {
      x[3 + i] = this->Mesh->GetPointData()->GetTCoords()->GetComponent(ptId, i - this->AttributeComponents[2]) *
                 this->AttributeScale[3];
    } else if (i < this->AttributeComponents[4]) {
      x[3 + i] = this->Mesh->GetPointData()->GetTensors()->GetComponent(ptId, i - this->AttributeComponents[3]) *
...
//----------------------------------------------------------------------------
int vtkQuadricDecimation2::RequestData(vtkInformation* vtkNotUsed(request), vtkInformationVector** inputVector,
  ...
  if (input->GetPolys() == NULL || input->GetPoints() == NULL || input->GetPointData() == NULL ||
      input->GetFieldData() == NULL) {
  ...
  this->ErrorQuadrics = new vtkQuadricDecimation2::ErrorQuadric[numPts];
  ...
  int abort = 0;
  while (!abort && edgeId >= 0 && cost < VTK_DOUBLE_MAX && this->ActualReduction < this->TargetReduction) {
  ...
  vtkDebugMacro(<< "Number Of Edge Collapses: " << this->NumberOfEdgeCollapses << " Cost: " << cost);
      ...
      if (vtkMath::LUFactorLinearSystem(A, index, 4)) {
        for (i = 0; i < this->NumberOfComponents; i++) {
          ...
          if (i < this->AttributeComponents[0]) {
            ...
            x[0] = input->GetPointData()->GetVectors()->GetComponent(pts[0], i - this->AttributeComponents[0]) *
                   this->AttributeScale[1];
            x[1] = input->GetPointData()->GetVectors()->GetComponent(pts[1], i - this->AttributeComponents[0]) *
                   this->AttributeScale[1];
            x[2] = input->GetPointData()->GetVectors()->GetComponent(pts[2], i - this->AttributeComponents[0]) *
                   this->AttributeScale[1];
          } else if (i < this->AttributeComponents[2]) {
            x[0] = input->GetPointData()->GetNormals()->GetComponent(pts[0], i - this->AttributeComponents[1]) *
                   this->AttributeScale[2];
            x[1] = input->GetPointData()->GetNormals()->GetComponent(pts[1], i - this->AttributeComponents[1]) *
                   this->AttributeScale[2];
            x[2] = input->GetPointData()->GetNormals()->GetComponent(pts[2], i - this->AttributeComponents[1]) *
                   this->AttributeScale[2];
          } else if (i < this->AttributeComponents[3]) {
            x[0] = input->GetPointData()->GetTCoords()->GetComponent(pts[0], i - this->AttributeComponents[2]) *
                   this->AttributeScale[3];
            x[1] = input->GetPointData()->GetTCoords()->GetComponent(pts[1], i - this->AttributeComponents[2]) *
                   this->AttributeScale[3];
            x[2] = input->GetPointData()->GetTCoords()->GetComponent(pts[2], i - this->AttributeComponents[2]) *
                   this->AttributeScale[3];
          } else if (i < this->AttributeComponents[4]) {
            x[0] = input->GetPointData()->GetTensors()->GetComponent(pts[0], i - this->AttributeComponents[3]) *
                   this->AttributeScale[4];
            x[1] = input->GetPointData()->GetTensors()->GetComponent(pts[1], i - this->AttributeComponents[3]) *
                   this->AttributeScale[4];
            x[2] = input->GetPointData()->GetTensors()->GetComponent(pts[2], i - this->AttributeComponents[3]) *
                   this->AttributeScale[4];
          }
...
//----------------------------------------------------------------------------
void vtkQuadricDecimation2::FindAffectedEdges(vtkIdType p1Id, vtkIdType p2Id, vtkIdList* edges) {
...
void vtkQuadricDecimation2::UpdateEdgeData(vtkIdType pt0Id, vtkIdType pt1Id) {
  ...
  for (i = 0; i < changedEdges->GetNumberOfIds(); i++) {
    ...
    if (edge[0] == pt1Id) {
      if (this->Edges->IsEdge(edge[1], pt0Id) == -1) {  // The edge will be completely new, add it.
        edgeId = this->Edges->GetNumberOfEdges();
    ...
    } else if (edge[1] == pt1Id) {  // The edge will be completely new, add it.
    ...
    } else {  // This edge already has one point as the merged point.
      if (this->AttributeErrorMetric) {
...
double vtkQuadricDecimation2::ComputeCost(vtkIdType edgeId, double* x) {
  ...
  for (i = 0; i < 11 + 4 * this->NumberOfComponents; i++) {
    this->TempQuad[i] = this->ErrorQuadrics[pointIds[0]].Quadric[i] + this->ErrorQuadrics[pointIds[1]].Quadric[i];
  }
  ...
  if (fabs(vtkMath::Determinant3x3(A)) / (norm * norm * norm) > errorNumber) {
    ...
    if (vtkMath::Dot(temp2, temp2) > errorNumber) {
      ...
      for (i = 0; i < 3; i++) temp[i] = b[i] - temp[i];
      c = vtkMath::Dot(temp2, temp) / vtkMath::Dot(temp2, temp2);
      for (i = 0; i < 3; i++) x[i] = pt1[i] + c * v[i];
    } else {
      // use mid point
...
int vtkQuadricDecimation2::TrianglePlaneCheck(const double t0[3], const double t1[3], const double t2[3],
                                              const double* x) {
...
int vtkQuadricDecimation2::IsGoodPlacement(vtkIdType pt0Id, vtkIdType pt1Id, const double* x) {
