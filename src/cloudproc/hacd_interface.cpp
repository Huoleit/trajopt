...
using namespace pcl;
using namespace std;
void polygonMeshFromPointsTriangles(pcl::PolygonMesh& mesh, const vector<Vec3<Real> >& points,
...
void pointsTrianglesFromPolygonMesh(const pcl::PolygonMesh& mesh, vector<Vec3<Real> >& points,
...
vector<pcl::PolygonMesh::Ptr> ConvexDecompHACD(const pcl::PolygonMesh& mesh, float concavity) {
  //  std::cout << "Usage: ./testHACD fileName.off minNClusters maxConcavity invertInputFaces addExtraDistPoints
  //  addFacesPoints ConnectedComponentsDist targetNTrianglesDecimatedMesh"<< std::endl; std::cout << "Recommended
  //  parameters: ./testHACD fileName.off 2 100 0 1 1 30 2000"<< std::endl;
