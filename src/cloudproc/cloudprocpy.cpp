...
template <>
const char* type_traits<float>::npname = "float32";
template <>
const char* type_traits<int>::npname = "int32";
template <>
const char* type_traits<uint8_t>::npname = "uint8";
template <>
const char* type_traits<uint16_t>::npname = "uint16";
...
struct PyCloud {
  ...
  static py::object to3dArray(PointCloudT* cloud) {
    return toNdarray3<float>((const float*)cloud->points.data(), cloud->height, cloud->width,
  ...
  static void save(PointCloudT* cloud, const std::string& fname) { saveCloud(*cloud, fname); }
...
py::object pyConvexDecompHACD(const PolygonMesh& mesh, float concavity) {
  ...
  py::list out;
  BOOST_FOREACH (const PolygonMesh::Ptr& convexmesh, convexmeshes) { out.append(convexmesh); }
  return out;
}
...
BOOST_PYTHON_MODULE(cloudprocpy) {
  ...
  py::def("integralNormalEstimation", &integralNormalEstimation,
          (py::arg("cloud"), py::arg("maxDepthChangeFactor") = .02, py::arg("normalSmoothingSize") = 10.0));
  ...
  py::class_<CloudGrabber, boost::shared_ptr<CloudGrabber> >("CloudGrabber")
      ...
      .def("getXYZ", &CloudGrabber::getXYZ,
           "Wait for new XYZ point cloud and return it. If not streaming, will start and stop. (and hang a couple "
           "seconds)")
  ...
  py::def("convexDecompHACD", &pyConvexDecompHACD, (py::arg("concavityParam") = 100),
          "input: mesh. output: list of meshes. concavityParam: see "
