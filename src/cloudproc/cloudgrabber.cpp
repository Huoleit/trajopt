...
class CloudGrabberImpl {
 public:
  enum StreamingState { IDLE, XYZ, XYZRGB, RGBD };
  enum RequestState { NONE, PENDING };
  ...
  void rgbd_cb(const boost::shared_ptr<openni_wrapper::Image>& rgb,
...
pcl::PointCloud<pcl::PointXYZ>::Ptr CloudGrabberImpl::getXYZ() {
  if (m_ss == IDLE) {
    ...
    PRINT_AND_THROW("asked for xyz but currently streaming another type");
}
pcl::PointCloud<pcl::PointXYZRGB>::Ptr CloudGrabberImpl::getXYZRGB() {
  if (m_ss == IDLE) {
    ...
    PRINT_AND_THROW("asked for xyzrgb but currently streaming another type");
}
RGBD::Ptr CloudGrabberImpl::getRGBD() {
  if (m_ss == IDLE) {
    ...
    PRINT_AND_THROW("asked for rgbd but currently streaming another type");
}
...
void CloudGrabberImpl::startRGBD() {
  ...
  boost::function<void(const boost::shared_ptr<openni_wrapper::Image>&,
      ...
      f = boost::bind(&CloudGrabberImpl::rgbd_cb, this, _1, _2);
  c = m_interface.registerCallback(f);
  m_interface.start();
}
