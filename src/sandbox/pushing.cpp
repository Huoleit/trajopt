...
enum ContactType { SLIDING, STICKING };
...
struct Face {
  ...
  Face() {}
  Face(const MatrixX2d& _poly, const OpenRAVE::Transform& _Tlf, KinBody::LinkPtr _link)
      : poly(_poly), Tlf(_Tlf), link(_link) {}
};
...
struct FaceContactErrorCalculator : public VectorOfVector {
  ...
  void CalcWorldPointsAndNormal(const VectorXd& x, OR::Vector& ptWorldA, OR::Vector& ptWorldB, OR::Vector& nWorldA,
                                OR::Vector& nWorldB) const {
    ...
    nWorldA = geometry::quatRotate(geometry::quatMultiply(m_faceA.link->GetTransform().rot, m_faceA.Tlf.rot),
                                   OR::Vector(0, 0, 1));
    nWorldB = geometry::quatRotate(geometry::quatMultiply(m_faceB.link->GetTransform().rot, m_faceB.Tlf.rot),
                                   OR::Vector(0, 0, 1));
  }
...
struct FaceContactConstraint : public ConstraintFromFunc, public Plotter {
  FaceContactConstraint(ConfigurationPtr config, const VarVector& dofvars, const VarVector& ptAvars,
                        const VarVector& ptBvars, const Face& faceA, const Face& faceB)
      : ConstraintFromFunc(VectorOfVectorPtr(new FaceContactErrorCalculator(config, faceA, faceB)),
                           concat(dofvars, ptAvars, ptBvars), VectorXd::Zero(0), EQ,
                           faceA.link->GetName() + "_" + faceB.link->GetName() + "contact") {}

  void Plot(const DblVec& xvec, OR::EnvironmentBase& env, std::vector<OR::GraphHandlePtr>& handles);
};

void FaceContactConstraint::Plot(const DblVec& xvec, OR::EnvironmentBase& env,
...
struct ForceBalanceConstraint : public EqConstraint {
  ...
  ForceBalanceConstraint(ConfigurationPtr config, KinBody::LinkPtr link, const vector<ContactPtr>& contacts,
                         const VarVector& dofs)
      : m_config(config), m_link(link), m_contacts(contacts), m_dofs(dofs) {
    name_ = "forcebalance";
  }
...
VectorXd SlidingFrictionErrorCalc::operator()(const VectorXd& x) const {
  ...
  OR::Vector localvel0 = geometry::quatRotate(
      geometry::quatMultiply(geometry::quatInverse(m_face.Tlf.rot), geometry::quatInverse(m_link->GetTransform().rot)),
      (p1world - p0world));
...
struct MechanicsProblem : public OptProb {
  ...
  MechanicsProblem(int nSteps, const vector<KinBodyPtr>& dynamicBodies, const vector<KinBodyPtr>& staticBodies,
                   RobotAndDOFPtr robotConfig);
 ...
 private:
  void AddContact(int iStep, const Face& faceA, const Face& faceB, const VarVector& ptAvars, const VarVector& ptBvars,
                  const Var& fn, const VarVector& ffr);
  void AddContactVariables(ContactType ctype, int iFirst, int iLast, const Face& faceA, const Face& faceB,
                           VarArray& ptAVars, VarArray& ptBVars, VarArray& fnVars, VarArray& ffrVars);
...
ConfigurationPtr MechanicsProblem::GetConfig(KinBody::LinkPtr link, int timestep) {
  ...
  for (int i = 0; i < m_dynBodies.size(); ++i)
    if (m_dynBodies[i] == body) return m_dynObjConfigs(timestep, i);
  for (int i = 0; i < m_staticBodies.size(); ++i)
    if (m_staticBodies[i] == body) return m_staticObjConfigs[i];
...
VarVector MechanicsProblem::GetDOFVars(KinBody::LinkPtr link, int timestep) {
  ...
  for (int i = 0; i < m_dynBodies.size(); ++i)
    if (m_dynBodies[i] == body) return m_obj2posvars[i].row(timestep);
  for (int i = 0; i < m_staticBodies.size(); ++i)
    if (m_staticBodies[i] == body) return VarVector();
  assert(0);
  return VarVector();
}

MechanicsProblem::MechanicsProblem(int nSteps, const vector<KinBodyPtr>& dynBodies,
                                   const vector<KinBodyPtr>& staticBodies, RobotAndDOFPtr robotConfig)
    : m_nSteps(nSteps), m_robotConfig(robotConfig), m_dynBodies(dynBodies), m_staticBodies(staticBodies) {
...
void MechanicsProblem::AddContacts(ContactType ctype, int iFirst, int iLast, const Face& faceA, const Face& faceB) {
  ...
  if (ctype == SLIDING) {
    for (int iStep = iFirst; iStep < iLast; ++iStep) {
      ...
      VarVector vars =
          concat(compositeDOFVars0, compositeDOFVars1, ptAVars.row(iStep), ffrVars.row(iStep), fnVars.row(iStep));
...
void MechanicsProblem::AddContact(int iStep, const Face& faceA, const Face& faceB, const VarVector& ptAVars,
                                  const VarVector& ptBVars, const Var& fn, const VarVector& ffr) {
  ConfigurationPtr configA = GetConfig(faceA.link, iStep);
  ConfigurationPtr configB = GetConfig(faceB.link, iStep);
  ConfigurationPtr compositeConfig(new CompositeConfig(configA, configB));
  ...
  addConstraint(
      ConstraintPtr(new FaceContactConstraint(compositeConfig, compositeDOFVars, ptAVars, ptBVars, faceA, faceB)));
  m_timestep2contacts[iStep].push_back(contact);
}

void MechanicsProblem::AddContactVariables(ContactType ctype, int iFirst, int iLast, const Face& faceA,
                                           const Face& faceB, VarArray& ptAVars, VarArray& ptBVars, VarArray& fnVars,
                                           VarArray& ffrVars) {
  if (ctype == SLIDING) {
    vector<int> cols;
    cols += 1, 2, 2, 2;
    vector<string> names;
    names += "fn", "ffr", "ptA", "ptB";
    vector<VarArray*> arrayPtrs;
    arrayPtrs += &fnVars, &ffrVars, &ptAVars, &ptBVars;
  ...
  for (int i = iFirst; i <= iLast; ++i) {
    ...
    // addLinearConstr(- mu * fnVars(i,0) - ffrVars(i,1), INEQ);
    getModel()->addIneqCnt(
        exprSquare(ffrVars(i, 0)) + exprSquare(ffrVars(i, 1)) - exprMult(exprSquare(fnVars(i, 0)), mu), "fc");
  }
...
void MechanicsProblem::AddMotionCosts(float jointvel_coeff, float obj_linvel_coeff, float obj_angvel_coeff) {
  ...
  for (int i = 0; i < m_dynBodies.size(); ++i) {
    ...
    if (obj_linvel_coeff > 0)
      addCost(
          CostPtr(new JointVelCost(m_obj2posvars[i].block(0, 0, m_nSteps, 3), obj_linvel_coeff * VectorXd::Ones(3))));
...
void PlotFace(EnvironmentBase& env, const Face& face, vector<OR::GraphHandlePtr>& handles) {
  ...
  handles.push_back(
      env.drawlinestrip(ptdata.data(), ptdata.rows(), sizeof(float) * 3, 4, OR::RaveVector<float>(1, 0, 0, 1)));
  ...
  handles.push_back(
      env.drawarrow(centerworld, centerworld + .05 * normalworld, .001, OR::RaveVector<float>(1, 1, 0, 1)));
}
...
void MechanicsProblem::Callback(DblVec& x) {
  ...
  for (int iObj = 0; iObj < m_dynBodies.size(); ++iObj) {
    ...
    for (int iStep = 0; iStep < q.rows(); ++iStep) {
      ...
      // cout << quatMult(quatExp(rvals.row(iStep)), q.row(iStep)).transpose() << " =?= " <<
      // geometry::quatMultiply(geometry::quatFromAxisAngle(toRave(rvals.row(iStep))), toRaveQuat(q.row(iStep))) <<
      // endl; cout << quatExp(rvals.row(iStep)).transpose() << " =?= " <<
      // OR::geometry::quatFromAxisAngle(toRave(rvals.row(iStep))) << endl;
  ...
  for (int iStep = 0; iStep < m_nSteps; ++iStep) {
    ...
    BOOST_FOREACH (ContactPtr contact, contacts) {
      if (contact->m_faceA.link->GetParent() == robot)
        PlotFace(*env, contact->m_faceA, handles);
      else if (contact->m_faceB.link->GetParent() == robot)
...
void SetupPR2Push(EnvironmentBasePtr env, boost::shared_ptr<MechanicsProblem>& prob, DblVec& xinit) {
  ...
  for (int i = 0; i < nSteps; ++i) {
    for (int j = 0; j < 6; ++j) {
      ...
      if (constrain_all_poses || (i == 0) || (i == nSteps - 1))
        prob->addLinearConstraint(exprSub(AffExpr(posvars(i, j)), ptarg), EQ);
    }
  }
}
...
4. is interesting. Just initialize it with a contact between its finger and the top. and tell the optimizer it needs to
slide the thing out. so it'll first just try to push down and slide it. Let's first have a maybe top contact, and then
we have two maybe side contacts.
...
int main(int argc, char** argv) {
  vector<string> validProblems;
  validProblems += "3linkpush", "pr2push", "pr2lift";
  ...
  config.add(new Parameter<bool>("constrain_all_poses", &constrain_all_poses,
                                 "constrain pose trajectory, as opposed to just start and end"));
