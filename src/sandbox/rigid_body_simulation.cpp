...
void AddInsideConstraints(OptProb& prob, const VarArray& localpts, KinBody::LinkPtr link) {
  ...
  class ContactComplementarity : public EqConstraint {
    ...
    ContactComplementarity(const VarVector& originA, const VarVector& localA, const VarVector& rotIncA,
                           const Vector4d& rotationA, const VarVector& originB, const VarVector& localB,
                           const VarVector& rotIncB, const Vector4d& rotationB, const Var& fn)
        : m_originA(originA),
          m_localA(localA),
          m_rotIncA(rotIncA),
  ...
  ContactTraj::ContactTraj(RigidBodyTrajPtr trajA, RigidBodyTrajPtr trajB, OptProb & prob, int n_steps, double dt)
      : m_dt(dt) {
    ...
    for (int i = 0; i < n_steps; ++i) {
      prob.addConstr(ConstraintPtr(new ContactComplementarity(trajA->m_x.row(i), m_pA.row(i), trajA->m_r.row(i),
                                                              trajA->m_q.row(i), trajB->m_x.row(i), m_pB.row(i),
                                                              trajB->m_r.row(i), trajB->m_q.row(i))));
    }
  }
  ...
  void Callback(OptProb * prob, DblVec & x) { dynamic_cast<RigidBodySim*>(prob)->Callback(x); }
  ...
  0; virtual DblVec RandomDOFValues() = 0;
  };
  */
  ...
  class RigidBody : public Configuration {
    ...
    void SetQ(const Vector4d q) { m_q = q; }
    virtual void SetDOFValues(const DblVec& dofs) = 0;
    ...
    virtual bool DoesAffect(const KinBody::Link& link) { return link == m_link; }
    virtual std::vector<KinBody::LinkPtr> GetAffectedLinks() { return std::vector<KinBody::LinkPtr> out(1, m_link); }
    virtual void GetAffectedLinks(std::vector<KinBody::LinkPtr>& links, bool only_with_geom, vector<int>& link_inds) {
      links = GetAffectedLinks();
      link_inds = vector<int>(1, 0);
    }
    virtual DblVec RandomDOFValues() { PRINT_AND_THROW("not implemented"); }
  };
  ...
  class CompositeConfig : public Configuration {
   public:
    ...
    virtual int GetDOF() const {
      int out = 0;
      BOOST_FOREACH (ConfigPtr& config, m_configs) { out += config->GetDOF(); }
      return out;
    }
    ...
    virtual DblMatrix PositionJacobian(int link_ind, const OR::Vector& pt) const {
      ...
      out.block(0, i_startDof, 3, m_configs[i_part]->GetDOF()) =
          m_configs[i_part]->PositionJacobian(link_ind % PART_FACTOR, pt);
    }
    ...
    virtual bool DoesAffect(const KinBody::Link& link) {
      bool out = false;
      BOOST_FOREACH (ConfigPtr& config, m_configs) { out |= config->DoesAffect(link); }
      return out;
    };
    ...
    virtual void GetAffectedLinks(std::vector<KinBody::LinkPtr>& links, bool only_with_geom, vector<int>& link_inds) {
      ...
      BOOST_FOREACH (ConfigPtr& config, m_configs) {
        ...
        BOOST_FOREACH (int& i, part_link_inds)
          i += PART_FACTOR * i_part;
    ...
    SaverPtr Save() { return SaverPtr(new CompositeSaver(this)); }
  };
  /**
  ...
  What's the abstraction to put around rigid bodies? At each timestep we have a different Configuration. On update, we
  set the base quaternion.
