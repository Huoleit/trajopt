...
struct NeedleError : public VectorOfVector {
  ...
  double radius;
  KinBodyPtr body;
  NeedleError(ConfigurationPtr cfg0, ConfigurationPtr cfg1, double radius)
      : cfg0(cfg0), cfg1(cfg1), radius(radius), body(cfg0->GetBodies()[0]) {}
...
int main(int argc, char** argv) {
  ...
  Var dthetavar =
      prob->createVariables(singleton<string>("speed"), singleton<double>(dtheta_lb), singleton<double>(INFINITY))[0];
  ...
  for (int i = 0; i < n_steps - 1; ++i) {
    ...
    prob->addConstraint(
        ConstraintPtr(new ConstraintFromFunc(f, vars, coeffs, EQ, (boost::format("needle%i") % i).str())));
  }
