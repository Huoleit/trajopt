...
class CompositeConfig : public Configuration {
 public:
  ...
  CompositeConfig(const vector<ConfigurationPtr>& configs) { Init(configs); }
  ...
  virtual int GetDOF() const {
    int out = 0;
    BOOST_FOREACH (const ConfigurationPtr& config, m_configs) { out += config->GetDOF(); }
    return out;
  }
  ...
  virtual bool DoesAffect(const KinBody::Link& link) {
    bool out = false;
    BOOST_FOREACH (ConfigurationPtr& config, m_configs) { out |= config->DoesAffect(link); }
    return out;
  };
