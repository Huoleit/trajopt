...
int main(int argc, char** argv) {
  ...
  if (envfile == "jagged_narrow.xml") {
    ...
    //    for (int i=0; i < bodies.size(); ++i) if (bodies[i]->GetName() == "obstacle")
    //    viewer->SetTransparency(bodies[i], .5);
  ...
  Var lengthvar =
      prob->createVariables(singleton<string>("speed"), singleton<double>(length_lb), singleton<double>(INFINITY))[0];
