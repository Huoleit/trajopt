...
struct DistFromPt : public ScalarOfVector {
  ...
  double operator()(const VectorXd& x) const { return (x - pt_).squaredNorm(); }
};

struct g0 : public VectorOfVector {
  VectorXd operator()(const VectorXd& x) const { return x; }
};
...
int main(int argc, char** argv) {
  ...
  prob->addCost(
      CostPtr(new CostFromFunc(ScalarOfVectorPtr(new DistFromPt(Vector2d(2, 1))), prob->getVars(), "distfunc")));
  prob->addConstraint(
      ConstraintPtr(new ComplCnt(INEQ, VectorOfVectorPtr(new g0()), VectorOfVectorPtr(new h0()), x0, y0, 1e-4)));
