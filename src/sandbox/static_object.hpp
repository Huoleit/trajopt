...
struct StaticObject : public Configuration {
  ...
  StaticObject(OpenRAVE::KinBodyPtr body) : m_body(body) {}
  ...
  virtual OpenRAVE::EnvironmentBasePtr GetEnv() { return m_body->GetEnv(); }
  virtual DblMatrix PositionJacobian(int link_ind, const OR::Vector& pt) const { return MatrixXd::Zero(3, 0); }
  ...
  virtual bool DoesAffect(const KinBody::Link& link) {
    BOOST_FOREACH (const KinBody::LinkPtr& link1, m_body->GetLinks()) {
      if (link1.get() == &link) return true;
    }
    return false;
  }
  virtual std::vector<KinBody::LinkPtr> GetAffectedLinks() { return m_body->GetLinks(); }
  virtual void GetAffectedLinks(std::vector<OpenRAVE::KinBody::LinkPtr>& links, bool only_with_geom,
  ...
  virtual DblVec RandomDOFValues() { return toDblVec(VectorXd::Random(0)); }
