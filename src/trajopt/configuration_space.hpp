...
class TRAJOPT_API Configuration {
 public:
  ...
  struct GenericSaver : public Saver {
    ...
    ~GenericSaver() { parent->SetDOFValues(dofvals); }
  };  // inefficient

  virtual SaverPtr Save() { return SaverPtr(new GenericSaver(this)); }
};
...
class TRAJOPT_API RobotAndDOF : public Configuration {
 public:
  RobotAndDOF(OR::KinBodyPtr _robot, const IntVec& _joint_inds, int _affinedofs = 0,
  ...
  SaverPtr Save() { return SaverPtr(new RobotSaver(robot)); }
