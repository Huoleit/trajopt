...
inline OR::Transform toRaveTransform(const Vector4d& q, const Vector3d& p) {
  return OR::Transform(OR::Vector(q[0], q[1], q[2], q[3]), OR::Vector(p[0], p[1], p[2]));
}
...
void TRAJOPT_API AddVarArrays(OptProb& prob, int rows, const vector<int>& cols, const vector<string>& name_prefix,
