...
void CollisionChecker::IgnoreZeroStateSelfCollisions(OpenRAVE::KinBodyPtr body) {
  ...
  for (int i = 0; i < collisions.size(); ++i) {
    LOG_DEBUG("ignoring self-collision: %s %s", collisions[i].linkA->GetName().c_str(),
              collisions[i].linkB->GetName().c_str());
...
void CollisionChecker::IgnoreZeroStateSelfCollisions() {
  ...
  BOOST_FOREACH (const KinBodyPtr& body, bodies) { IgnoreZeroStateSelfCollisions(body); }
}

std::ostream& operator<<(std::ostream& o, const Collision& c) {
  o << (c.linkA ? c.linkA->GetName() : "NULL") << "--" << (c.linkB ? c.linkB->GetName() : "NULL")
    << " distance: " << c.distance << " normal: " << c.normalB2A << " ptA: " << c.ptA << " ptB: " << c.ptB
    << " time: " << c.time << " weight: " << c.weight;
  return o;
}
