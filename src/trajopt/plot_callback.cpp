...
void PlotCosts(OSGViewer& viewer, const vector<CostPtr>& costs, const vector<ConstraintPtr>& cnts, Configuration& rad,
               const VarArray& vars, const DblVec& x) {
...
Optimizer::Callback PlotCallback(TrajOptProb& prob) {
  ...
  return boost::bind(&PlotCosts, boost::ref(*viewer), boost::ref(prob.getCosts()), cnts, boost::ref(*prob.GetRAD()),
                     boost::ref(prob.GetVars()), _2);
}
