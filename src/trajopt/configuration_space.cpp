...
// TODO: configuration should know something about what dofs are part of SO(1), SO(2) etc for functions like
// RandomDOFValues
...
void RobotAndDOF::SetDOFValues(const DblVec& dofs) {
  if (affinedofs != 0) {
    ...
    if (joint_inds.size() > 0) robot->SetDOFValues(dofs, false, joint_inds);
  } else {
...
DblVec RobotAndDOF::GetDOFValues() {
  DblVec out;
  if (joint_inds.size() > 0) robot->GetDOFValues(out, joint_inds);
  if (affinedofs != 0) {
    out.resize(GetDOF());
...
void RobotAndDOF::SetRobotActiveDOFs() {
  ...
  if (robot1->GetActiveDOF() != GetDOF() ||
      !std::equal(current_active.begin(), current_active.end(), joint_inds.begin()))
    robot1->SetActiveDOFs(joint_inds, affinedofs);
}

void RobotAndDOF::GetDOFLimits(DblVec& lower, DblVec& upper) const {
  if (joint_inds.size() > 0) robot->GetDOFLimits(lower, upper, joint_inds);
