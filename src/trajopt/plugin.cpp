/** \example plugincpp.cpp
   ...
   Every plugin contains a bunch of openrave interfaces, the plugincpp plugin creates a simple OpenRAVE::ModuleBase
   interface named \b mymodule.
   ...
   If you are using octave or matlab, then can communicate with openrave through tcp/ip, check out:
   ...
   drops into the python promp with the plugin loaded and a scene loaded. Then it is possible to execute the following
   python commands to create the interface and call a command:
