...
static vector<ConvexConstraintsPtr> convexifyConstraints(const vector<ConstraintPtr>& cnts, const DblVec& x,
                                                         Model* model) {
...
void printLinearConstraintsInfo(const vector<double>& old_cnt_vals, const vector<double>& new_cnt_vals,
  ...
  for (size_t i = 0; i < old_cnt_vals.size(); ++i) {
    ...
    if (fabs(old_cnt_vals[i]) > 1e-8)
      printf("%15s | %10.3e | %10.3e | %10.3e | %10.3e\n", cnt_names[i].c_str(), old_cnt_vals[i], new_cnt_vals[i],
             exact_improve, exact_improve / old_cnt_vals[i]);
    else
      printf("%15s | %10.3e | %10.3e | %10.3e | %10s\n", cnt_names[i].c_str(), old_cnt_vals[i], new_cnt_vals[i],
             exact_improve, "  ------  ");
  }
...
vector<ConvexObjectivePtr> cntsToCosts(const vector<ConvexConstraintsPtr>& cnts, double err_coeff, Model* model) {
  ...
  BOOST_FOREACH (const ConvexConstraintsPtr& cnt, cnts) {
    ...
    BOOST_FOREACH (const AffExpr& aff, cnt->eqs_) { obj->addAbs(aff, err_coeff); }
    BOOST_FOREACH (const AffExpr& aff, cnt->ineqs_) { obj->addHinge(aff, err_coeff); }
    out.push_back(obj);
  }
  return out;
}
...
OptStatus BasicTrustRegionSQP::optimize() {
  ...
  for (int merit_increases = 0; merit_increases < max_merit_coeff_increases_;
    ...
    for (int iter = 1;; ++iter) { /* sqp loop */
      ...
      while (trust_box_size_ >= min_trust_box_size_) {
        ...
        if (status != CVX_SOLVED) {
          LOG_ERROR(
              "convex solver failed! set TRAJOPT_LOG_THRESH=DEBUG to see solver output. saving model to /tmp/fail.lp "
              "and IIS to /tmp/fail.ilp");
        ...
        if (approx_merit_improve < -1e-5) {
          LOG_ERROR("approximate merit function got worse (%.3e). (convexification is probably wrong to zeroth order)",
