...
Matrix3d toMatrix3d(const OpenRAVE::TransformMatrix& tm) {
  Matrix3d out;
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j) out(i, j) = tm.rot(i, j);
  return out;
}
...
OpenRAVE::Transform toRave(const rbdmath::SpatialTransform& T) {
  ...
  tm.trans = toRave(T.r);
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j) tm.rot(i, j) = T.E(j, i);
...
boost::shared_ptr<rbd::Model> MakeRBDLModel(OpenRAVE::RobotBasePtr robot, bool floating_base, Link2ID& link2id) {
  ...
  KinBody::LinkPtr base_link =
      robot->GetJoints().empty() ? robot->GetLinks()[0] : robot->GetJoints()[0]->GetHierarchyParentLink();
  unsigned base_id = 0;
  if (floating_base)
    base_id = model->SetFloatingBaseBody(rbd::Body(base_link->GetMass(), toVector3d(base_link->GetLocalCOM()),
                                                   toMatrix3d(base_link->GetLocalInertia())));
  ...
  BOOST_FOREACH (const KinBody::JointPtr& joint, robot->GetJoints()) {
    KinBody::LinkPtr parent = joint->GetHierarchyParentLink(), child = joint->GetHierarchyChildLink();
    ...
    link2id[child] = model->AddBody(link2id[realParent], toSpatialTransform(link2Tjl[realParent] * T_l0_j), rbd_joint,
                                    body, child->GetName());
