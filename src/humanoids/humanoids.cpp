...
aabb.pos()-aabb.extents()
 */
const double footpolydata[8] = {0.17939, 0.062707, 0.17939, -0.061646, -0.08246, -0.061646, -0.08246, 0.061646};
 ...
 *
 */
const double footbottomdata[12] = {0.17939,  0.062707,  -0.0353015, 0.17939,  -0.061646, -0.0353015,
                                   -0.08246, -0.061646, -0.0353015, -0.08246, 0.062707,  -0.0353015};
...
void ZMPConstraint::Plot(const DblVec& x, OR::EnvironmentBase& env, std::vector<OR::GraphHandlePtr>& handles) {
  ...
  moment /= totalmass;
  OR::Vector moment_ground = moment;
  moment_ground.z = 0;
...
PECost::PECost(RobotAndDOFPtr rad, const VarVector& vars, double coeff)
    : CostFromErrFunc(VectorOfVectorPtr(new PECalc(rad)), vars, VectorXd::Ones(1) * coeff, SQUARED, "PE") {}
...
struct FootHeightCalc : public VectorOfVector {
  ...
  double m_height;
  FootHeightCalc(const RobotAndDOFPtr rad, KinBody::LinkPtr link, double height)
      : m_rad(rad), m_link(link), m_height(height) {}
  VectorXd operator()(const VectorXd& x) const {
    m_rad->SetDOFValues(toDblVec(x));
    OpenRAVE::Transform T = m_link->GetTransform();
    Eigen::Vector4d out(m_height - (T * toRaveVector(footbottompoints.row(0))).z,
...
FootHeightConstraint::FootHeightConstraint(RobotAndDOFPtr rad, KinBody::LinkPtr link, double height,
                                           const VarVector& vars)
    : ConstraintFromFunc(VectorOfVectorPtr(new FootHeightCalc(rad, link, height)), vars, VectorXd::Ones(1), INEQ,
                         "FootHeight") {}
...
struct PECostInfo : public TermInfo, public MakesCost {
  ...
  void hatch(TrajOptProb& prob) {
    prob.addCost(
        CostPtr(new PECost(boost::dynamic_pointer_cast<RobotAndDOF>(prob.GetRAD()), prob.GetVarRow(timestep), coeff)));
...
struct StaticTorqueCostInfo : public TermInfo, public MakesCost {
  ...
  void hatch(TrajOptProb& prob) {
    prob.addCost(CostPtr(new StaticTorqueCost(boost::dynamic_pointer_cast<RobotAndDOF>(prob.GetRAD()),
                                              prob.GetVarRow(timestep), coeff)));
    prob.getCosts().back()->setName(name);
  }
